# 젠지 (ZENJI · 全字) — 모든 글자를 내 것으로

> 全(ぜん, 모든) + 字(じ, 글자): 일본어 문장의 모든 글자를 소리와 색으로 풀어 준다는 뜻.
...
- **실제 사용**: [Anthropic 콘솔](https://console.anthropic.com)에서 API 키 발급 → 설정 탭에 입력
  ...
  - 기본 모델은 **Haiku 4.5**(문장당 약 $0.005) — 분석 검증에 실패한 어려운 문장만
    자동으로 **Opus 5**로 승격해 재분석하므로 평소 비용은 최소, 품질은 유지됩니다
  - 같은 문장은 캐시되어 재분석 비용이 들지 않습니다
