...
4. Worker 화면 → **Settings** → **Variables and Secrets** 에서 추가:
   ...
   | `ADMIN_CODE` | **Secret** | 관리자용 코드 (설정 페이지에 입력하면 무제한) |
   | `DAILY_LIMIT` | Text | 사이트 전체 하루 분석 횟수 (예: `25`) |
...
- 로그인 없음 — 접속하면 바로 사용. **사이트 전체 공용으로 하루 `DAILY_LIMIT`회(기본 25)
  문장 분석** 가능. 한→일 번역·손글씨 인식 등 부가 호출은 횟수에 포함되지 않습니다.
  자정(KST) 리셋, 헤더에 "오늘 N/25회" 배지 표시.
- **설정 탭에 `ADMIN_CODE`를 입력**하면 그 브라우저는 **무제한** + "관리자 · 무제한" 표시.
- 한도 초과 시 "오늘 사이트의 분석 한도를 모두 사용했어요" 안내 후 자정까지 차단.
